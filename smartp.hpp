...
template <typename T> struct default_deleter_t {
...
template <typename T> struct default_deleter_t<T[]> {
  void operator()(T *pointer) {
#ifdef __linux__
    delete[] pointer;
#elif _WIN32
    ...
    for (size_t i = 0; i < size; ++i)
      pointer[i].~T();
    HeapFree(GetProcessHeap(), 0, reinterpret_cast<LPVOID>(real));
#endif
  }
};
template <typename T> struct default_allocator_t {
  template <typename... Args> T *operator()(Args &&...args) {
#ifdef __linux__
    return new T{forward<Args>(args)...};
#elif _WIN32
    auto ptr = reinterpret_cast<T *>(HeapAlloc(GetProcessHeap(), 0, sizeof(T)));
    new (ptr) T(forward<Args>(args)...);
    return ptr;
#endif
  }
};
template <typename T> struct default_allocator_t<T[]> {
...
template <typename T, typename D> class uniq_ptr;
...
template <typename _Ty, typename _Deleter> class uniq_ptr<_Ty[], _Deleter> {
public:
  using type = _Ty;
  using pointer = type *;
  using reference = type &;
...
template <typename _Ty> class uniq_ptr<_Ty[], default_deleter_t<_Ty[]>> {
public:
  using type = _Ty;
  using pointer = type *;
  using reference = type &;
...
template <typename T> void destruct(T *ptr) {
  ...
  delete[](reinterpret_cast<char *>(real));
}
...
public:
  ...
  array(array &&move)
      : m_data{forward<uniq_ptr<T[]>>(move.m_data)}, m_capacity{
                                                         move.m_capacity} {
    move.m_capacity = 0;
  }
  ...
  explicit array(T const *copy) noexcept
      : m_data{}, m_capacity{stringlen(copy) + 1} {
    m_data = make_uniq<T[]>(m_capacity);
    for (size_t i = 0; i < m_capacity; ++i)
      m_data[i] = copy[i];
    // memcpy(m_data.get(), copy, m_capacity * sizeof(T));
    // m_data[m_capacity-1] = static_cast<T>('\0');
  }
...
public:
  ...
  void append(T const &item) {
    if (size() == this->capacity()) {
      ...
      m_size = 0;
      for (auto &&itemo : *this)
        newdata[m_size++] = forward<T>(itemo);
